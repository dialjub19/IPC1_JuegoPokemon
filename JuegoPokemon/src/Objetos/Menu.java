...
public class Menu {
    ...
    private void pokemonMasFuerte() {
        ...
        for (int i = 0; i < this.contadorReporte; i++) {
            ...
            System.out.println(reporte[i].getMiPokemon().getImagen());
        }
    }

    // Este metodo muestra un listado del pokemon mas debiles basado en la cantidad de veces que fue elegido y la cantitdad de veces que a ganado.
    private void pokemonMasDebil() {
    ...
    private void registroPartida() {
        ...
        int contadorBatalla = 1;
        for (int i = 0; i < this.contadorJugador; i++) {
            System.out.println("\n.-.-.-.-.-.-.-. Batalla #" + (i + contadorBatalla) + " .-.-.-.-.-.-.-.-.-.");
            ...
            System.out.println(jugadorUno[i].getEstado() + " de la batalla ");
            ...
            System.out.println(jugadorDos[i].getEstado() + " de la batalla \n");
        }
    ...
    /* Este metodo recibe como parametro el nombre del pokemon para luego buscarlo en el arreglo y de encontrarlo retornara cada 
       atributo de este para luego crear un nuevo objeto y retornarlo, en caso contrario retornara un null. Se implemento de esta manera
       ya que si se retornaba la refetencia al objeto se veia afectado lo cual no puede suceder ya que solicitan que en cada partida 
       los pokemones vuelvan a tener sus puntos de vida iniciales. */
    private Pokemon elegirPokemon(String pokemon) {

        Scanner entrada = new Scanner(System.in);
        String nombre = "", estado = "", imagen = "";
        ...
        for (int i = 0; i < this.contador; i++) {
            if (this.poke[i].getNombre().equalsIgnoreCase(pokemon) && !(pokemon.equals("")))  {
                ...
                ataque = poke[i].getAtaque();
                imagen = poke[i].getImagen();
                partida = poke[i].getEstadoPartida();
                recibido = poke[i].getAtaqueRecibido();
                elegido = poke[i].getElegido();
